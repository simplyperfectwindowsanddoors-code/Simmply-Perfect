...
export default function Navbar() {
  ...
  const [mobileCompaniesOpen, setMobileCompaniesOpen] =
    useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rawDigits = formData.phone
      .slice(4)
      .replace(/\s/g, "");
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Failed to register document vault credentials.",
        );
      }
      ...
      setSubmitError(
        error.message || "Connection timeout exception.",
      );
